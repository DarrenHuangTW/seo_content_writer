...
def progress():
    ...
    def generate():
        ...
        semrush_data = process_semrush_data(df_results)
        yield format_sse_message("complete", step="semrush", title="SEMRush Data Retrieval and Processing", 
                               data={"json_data": [item for data in df_results['SEMRush_Data'] if data for item in data],
                                    "final_keywords_df": json.loads(semrush_data.to_json())})

        yield format_sse_message("progress", step="content", message="Fetching Content...")
        content_fetch_results = {}
        for index, row in df_results.iterrows():
            url = row['Link']
            content = fetch_content(url)
            content_fetch_results[url] = content
        yield format_sse_message("complete", step="content", title="Content Fetching", data=content_fetch_results)
